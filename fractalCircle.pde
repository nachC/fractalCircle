...
void setup() {
  ...
  drawCircles(r, coord, 0);
  
  popMatrix();
  println("Finished");
}

void drawCircles(float _r, ArrayList<float[]> _coord, int _count) {
  ...
  if(_count < numOfCycles) {
    ...
    _coord.subList(0, listSize).clear();
